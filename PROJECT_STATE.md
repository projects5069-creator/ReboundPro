...
- **🟡 פתוח/דחוי (הבא = H3):** **H3** intraday-WARN-ללא-מייל (escalation סטייטפולי ב-health_monitor, מייל על WARN↔OK) — כעת על **baseline-WARN נקי**; **intraday-rollup** dead-man יומי; **P2** (guard נגד דריסת intraday + טסטים ל-gradual_scanner); pytest ב-.venv; יישור cron/pinger (daily 21:30 מול 22:30); תזמון post (לא פרה-מרקט); הסרת ה-SA הישן של RidingHigh מה-Sheet.
