...
> ⚠️ **DEPRECATED (2026-06-26).** מקור-האמת התפעולי היחיד: **`docs/PROJECT_KNOWLEDGE.md`** (לא מתחזקים שני קבצי-state). הקובץ הזה נשמר כסנאפשוט היסטורי בלבד (קפוא 2026-06-16) ואינו מתעדכן.
...
- **✅ חידוד WARN שפיר — בוצע** (commit 421b2be, TDD, ראה MONITORING.md §1/§4): `scanner-freshness` מסומן **CALM** בחלון פרה-EOD (`last==היום` יום-מסחר ו-`now<18:30 ET` — intraday מקדים לגיטימית; סניף ה-FAIL "EOD לא רץ" נשמר); `volume-anomaly` מעריך **סשנים-שנסגרו בלבד** (מחריג את הסשן-הפתוח). אומת-חי 18.6 (scanner-freshness=CALM, volume-anomaly=OK מול ה-Sheet) · **suite 57/57**. מנקה את ה-baseline-WARN לקראת H3.
- **✅ P2 — בוצע** (TDD, ראה `tests/test_gradual_scanner.py`): כיסוי-טסטים ראשון ל-`gradual_scanner` (9 טסטים: סף **-10%/D-5** גבול, רצפת-נזילות price/cap/ADV, תיוג `gradual_drop`/`gradual_eod`, `recent_capture_set` dedup, ונעילת **אנטי-דריסה**), **bite-proven** (9 מראות-הפוכות נכשלו), **suite 66/66**. שאלת-הדריסה נחקרה: ה-dedup (`recent_capture_set`, דילוג לפני `build_snapshot`) כבר מגן **מבנית** מפני gradual שדורס שורת-intraday → **אין צורך ב-guard** (אפס שינוי קוד).
- **✅ H3 — קוד בוצע** (A+, transition-alert; ראה `health.yml` + `tests/test_health_verdict.py`): step `Verdict alert` מפנג `reboundpro-warn` `/fail` כש-verdict∈{WARN,FAIL} ו-`/success` כש-OK; hc מתריע **רק על מעבר** up↔down (מייל יחיד על OK→WARN ועל recovery, לא per-run). מנצל `Monitor.overall()`→exit-code (נעול ב-3 טסטים, bite-proven), **suite 69/69**. `reboundpro-health`/`reboundpro-daily` ללא שינוי (liveness). **guarded** — ה-step מדלג בשקט עד הקמת ה-secret. **נותרה הקמה-חיצונית (הבא):** check `reboundpro-warn` ב-healthchecks.io (**ללא-cron**, up/down טהור לפי verdict) + secret `HC_PING_WARN` + אימות e2e (curl /fail→מייל DOWN, /success→recovery).
- **🟡 פתוח/דחוי:** **intraday-rollup** dead-man יומי; ריכוך intraday-freshness ליום-שקט (אם ירעש את H3); pytest ב-.venv; יישור cron/pinger (daily 21:30 מול 22:30); תזמון post (לא פרה-מרקט); הסרת ה-SA הישן של RidingHigh מה-Sheet.
