...
- **מדדי-איסוף תיאוריים (פיצ'רים, לא אותות-כניסה; הכרעה ל-M4):**
  ...
  - **כיסוי:** שורות source=intraday מקבלות את **מלוא 8 שדות ה-context** (prior-decline M3.6 + 3 סממני-M3.7: vix_level/drop_day_rel_volume/sector_momentum) בריצת-ה-EOD דרך `scanner.backfill_intraday_prior_context`, שמאציל ל-`_context_for_row` (ROOT 2 forward-fix, `0aff067`, 2026-07-01; קודם כתב רק prior-decline). PIT (`≤scan_date`, ממשיכת-היסטוריה אחת, בלי Finviz) + **forward-only** (scoped ל-scan_date — בלי sweep היסטורי). שורות intraday היסטוריות מ-6/15–6/30 נשארות ריקות בסממנים עד ריפוי-היסטורי מאושר בנפרד (הכלי: `--backfill-context`).
