...
## ▶ NEXT — collection-leak follow-ups (2026-07-01)
*אבחון שני-שורשים read-only (ACTIONS_LOG 1/7). ROOT 1≠ROOT 2: תאריכי-נפילה 26/6 מול 15/6, Finviz-throttle מול פער-מבני; אי-התלות מאומתת בלוג 6/30 (fundamentals נכשל בעוד backfill הצליח באותה ריצה).*
- **ROOT 2 (intraday-context forward-fix)** — **DONE** (`0aff067`, TDD 7 טסטים, suite 194): שורות source=intraday מקבלות כעת אוטומטית vix_level/drop_day_rel_volume/sector_momentum דרך `backfill_intraday_prior_context`→`_context_for_row`, PIT + forward-only. אימות-חי = EOD 1/7 22:30 UTC.
- **ROOT 1 (fundamentals 0% מ-26/6)** — **ACCEPTED** (Finviz burst-throttle; אובדן היסטורי מקובל, לא בר-שחזור-PIT). המשך = הכרעת-Sharadar (GATE-B) — **דחוי, לא נגעתי**.
- **pending (a) — ריפוי היסטורי PIT ל-intraday-context** (6/15–6/30): החלטה נפרדת. הכלי קיים (`scanner.py --backfill-context`→`_context_for_row`) והוא PIT-נכון (VIX/vol/sector ≤scan_date, בניגוד ל-fundamentals). דורש אישור מפורש נפרד.
- **pending (b) — `intraday_timeseries` חצה ~74.8k שורות** → הטריגר לפיצול-חודשי/DB **הופעל** (ראה BACKLOG).
- **pending (c) — `research/historical/` → `.gitignore`** (תיקייה untracked, לא-קוד).
...
## 📦 BACKLOG
- **פיצול Google Sheets חודשי / מעבר ל-DB** — **⚠️ טריגר הופעל (1/7): `intraday_timeseries` חצה ~74.8k שורות** (>50k). שקול רוטציה חודשית (כמו RidingHigh) או מעבר ל-DB.
